...
/** @lucene.internal */
public class RandomAccessFile  implements Serializable {
//    CassandraRandomAccessFile thePrivateFile;
    ...
    public RandomAccessFile(File path, String string) throws FileNotFoundException {
        logger.info("RandomAccessFile(File path, String string {} )", string);
        //currentFile = path;
        //thePrivateFile = new CassandraRandomAccessFile(path.getFile(), string) ;
        //thePrivateFile = new CassandraRandomAccessFile(null, string, true, "lucene0", "index0", 16384) ;
        thePrivateFile = new java.io.RandomAccessFile(path.getFile(), string);
    }
    ...
    public void write(byte[] b, int offset, int n) throws IOException {  // Writes n bytes from the specified byte array starting at offset off to this file.
...
//    public CassandraRandomAccessFile getFile() { // TODO dont need
    ...
    public void getFDsync() throws SyncFailedException, IOException {
       ...
       thePrivateFile.getFD().sync(); // write into cassandra. // http://docs.oracle.com/javase/7/docs/api/java/io/FileDescriptor.html#sync()
