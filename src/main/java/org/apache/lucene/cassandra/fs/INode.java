...
public class INode {
    ...
    public INode(String user, String group, FsPermission perms,
            FileType fileType, Block[] blocks) {
    ...
    public INode(String user, String group, FsPermission perms,
            FileType fileType, Block[] blocks, long mtime) {
    ...
    public static INode deserialize(InputStream in, long ts) throws IOException {
        ...
        case DIRECTORY:
            in.close();
            return new INode(new String(ubuf), new String(gbuf), perms,
                    fileType, null, ts);
        case FILE:
            ...
            for (int i = 0; i < numBlocks; i++) {
                ...
                for (int j = 0; j < numSubBlocks; j++) {
                    ...
                    subBlocks[j] =
                            new SubBlock(new UUID(subMostSigBits,
                                    subLeastSigBits), subOffset, subLength);
                }

                blocks[i] =
                        new Block(new UUID(mostSigBits, leastSigBits), offset,
                                length, subBlocks);
            }
            in.close();
            return new INode(new String(ubuf), new String(gbuf), perms,
