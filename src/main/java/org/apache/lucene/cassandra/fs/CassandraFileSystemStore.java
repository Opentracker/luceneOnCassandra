...
public interface CassandraFileSystemStore {
     ...
     * Stores a SubBlock.
     * 
     * @param currentBlockUUID
     *            parent UUID used as row key for the subBlock row.
     * @param subBlock
     *            sub Block to be written
     * @param data
     *            content of the subBLock
     * @throws IOException
     *             if an error occur
     */
    void storeSubBlock(UUID currentBlockUUID, SubBlock subBlock, ByteBuffer data)
            throws IOException;

    INode retrieveINode(Path path) throws IOException;

    InputStream retrieveBlock(Block block, long byteRangeStart)
            throws IOException;
     ...
     * @param subBlock
     *            the subBLock to retrieve
     * @param byteRangeStart
     *            the offset where to stream the data from.
     * @return an inputStream to retrieve the content of the subBlock
     * @throws IOException
     *             if an error occurs
     */
    InputStream retrieveSubBlock(Block block, SubBlock subBlock,
     ...
     * 
     * @param inode
     *            parent inode of these subBlocks
     * @throws IOException
     *             if an error occurs
     */
    ...
    BlockLocation[] getBlockLocation(List<Block> usedBlocks, long start,
