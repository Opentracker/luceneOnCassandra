 ...
 * CFs schema:
 * 
 * Column Families: - inode - sblocks
 * 
 * ------------------- | inode | ------------------- {key : [<path>: < > ],
 * [<sentinel>: < >], [ <datacol> : < all blocks with its subBlocks serialized>]
 * }
 * 
 * ------------------ | sblocks | ------------------ { key(Block UUID):
 * [<subBlockUUID> : <data>>], [<subBlockUUID> : <data>>],
 * .......[<subBlockUUID> : <data>>] }
 */
public class CassandraFileSystemThriftStore implements CassandraFileSystemStore {
    private final static Logger logger = Logger
            .getLogger(CassandraFileSystemThriftStore.class);
    ...
    private static final ByteBuffer parentPathCol = ByteBufferUtil
            .bytes("parent_path");
    ...
    private static final SlicePredicate pathPredicate = new SlicePredicate()
            .setColumn_names(Arrays.asList(pathCol));
    ...
    public void initialize(URI uri, Configuration conf) throws IOException {
        ...
        if (conf instanceof CassandraJobConf)
            ...
            client =
                    CassandraProxyClient.newProxyConnection(host, port, true,
    ...
    /**
     * Set to different set of Column Families is the archive location is
     * selected.
     */
    ...
    private void initConsistencyLevels(KsDef ks, Configuration conf) {
        ...
        consistencyLevelRead =
                ConsistencyLevel.valueOf(conf.get(
    ...
    public KsDef createKeySpace() throws IOException {
        try {
            ...
            // jason : commented out below because not sure how to fix it yet.
            //cf.setMemtable_flush_after_mins(1);
            //cf.setMemtable_throughput_in_mb(128);

            cf.setColumn_metadata(Arrays.asList(
                    new ColumnDef(pathCol, "BytesType").setIndex_type(
                            IndexType.KEYS).setIndex_name("path"),
                    new ColumnDef(sentCol, "BytesType").setIndex_type(
                            IndexType.KEYS).setIndex_name("sentinel"),
                    new ColumnDef(parentPathCol, "BytesType").setIndex_type(
                            IndexType.KEYS).setIndex_name("parent_path")));
            ...
            // jason commented out becuase error. not sure how to fix it yet. 
            //cf.setMemtable_throughput_in_mb(128);
            //cf.setMemtable_flush_after_mins(1);
            ...
            stratOpts.put(BriskSimpleSnitch.BRISK_DC,
                    System.getProperty("cfs.replication", "1"));
    ...
    public InputStream retrieveSubBlock(Block block, SubBlock subBlock,
        ...
        try {
            blockData =
                    ((Brisk.Iface) client).get_cfs_sblock(FBUtilities
                            .getLocalAddress().getHostName(), blockId,
                            subBlockId, (int) 0, storageTypeInUse);
        } catch (Exception e) {
            throw new IOException(e);
        }
    ...
    private synchronized InputStream getInputStream(ByteBuffer bb)
            throws IOException {

        ByteBuffer output = null;

        if (compressedData == null
                || compressedData.capacity() < bb.remaining())
        ...
        if (Snappy.isValidCompressedBuffer(compressedData)) {
            ...
            if (uncompressedData == null
                    || uncompressedData.capacity() < uncompressedLength) {
                uncompressedData =
        ...
        try {
            ...
            MappedByteBuffer bb =
                    raf.getChannel().map(FileChannel.MapMode.READ_ONLY,
                            blockInfo.offset, blockInfo.length);
            ...
            throw new RuntimeException(String.format(
                    "Unable to mmap block %s[%d,%d]", blockInfo.file,
    ...
    public INode retrieveINode(Path path) throws IOException {
        ...
        if (pathInfo == null
                && consistencyLevelRead.equals(ConsistencyLevel.ONE)) {
            pathInfo =
                    performGet(pathKey, inodeDataPath, ConsistencyLevel.QUORUM);
        }
        ...
        return INode.deserialize(
                ByteBufferUtil.inputStream(pathInfo.column.value),
                pathInfo.column.getTimestamp());
    }

    private ColumnOrSuperColumn performGet(ByteBuffer key, ColumnPath cp,
            ConsistencyLevel cl) throws IOException {
     ...
     * {@inheritDoc}
     */
    public synchronized void storeSubBlock(UUID parentBlockUUID,
            SubBlock sblock, ByteBuffer data) throws IOException {
    ...
    public void storeINode(Path path, INode inode) throws IOException {
        ...
        // file name
        mutations.add(createMutationForCol(pathCol,
                ByteBufferUtil.bytes(path.toUri().getPath()), ts));

        // Parent name for this file
        mutations.add(createMutationForCol(parentPathCol,
                ByteBufferUtil.bytes(getParentForIndex(path)), ts));
     ...
     * @param path
     *            a Path
     * @return the parent to the <code>path</code> or null if the
     *         <code>path</code> represents the root.
     */
    ...
    /**
     * Creates a mutation for a column <code>colName</code> whose value is
     * <code>value</code> and with tiemstamp <code>ts</code>.
     * 
     * @param colName
     *            column name
     * @param value
     ...
     */
    private Mutation createMutationForCol(ByteBuffer colName, ByteBuffer value,
            long ts) {
        return new Mutation()
                .setColumn_or_supercolumn(new ColumnOrSuperColumn()
                        .setColumn(new Column().setName(colName)
                                .setValue(value).setTimestamp(ts)));
    }
    ...
    ByteBuffer getPathKey(Path path) {
        return ByteBufferUtil.bytes(FBUtilities.hashToBigInteger(
                ByteBufferUtil.bytes(path.toUri().getPath())).toString(16));
    }

    ByteBuffer uuidToByteBuffer(UUID id) {
        // jason : commented out below because not sure how to fix it yet.
        //return ByteBufferUtil.bytes(FBUtilities.bytesToHex(UUIDGen
        //        .decompose(id)));
        return null;
    }
    ...
    public void deleteINode(Path path) throws IOException {
        try {
            client.remove(getPathKey(path), inodePath,
    ...
    public Set<Path> listDeepSubPaths(Path path) throws IOException {
        ...
        List<IndexExpression> indexExpressions =
                new ArrayList<IndexExpression>();

        indexExpressions.add(new IndexExpression(sentCol, IndexOperator.EQ,
                sentinelValue));
        indexExpressions.add(new IndexExpression(pathCol, IndexOperator.GT,
                ByteBufferUtil.bytes(startPath)));

        // Limit listings to this root by incrementing the last char
        if (startPath.length() > 1) {
            String endPath =
                    startPath.substring(0, startPath.length() - 1)
                            + new Character((char) (startPath.charAt(startPath
                                    .length() - 1) + 1));

            indexExpressions.add(new IndexExpression(pathCol, IndexOperator.LT,
                    ByteBufferUtil.bytes(endPath)));
        }

        try {
            List<KeySlice> keys =
                    client.get_indexed_slices(inodeParent, new IndexClause(
                            indexExpressions, ByteBufferUtil.EMPTY_BYTE_BUFFER,
                            100000), pathPredicate, consistencyLevelRead);

            Set<Path> matches = new HashSet<Path>(keys.size());

            for (KeySlice key : keys) {
                for (ColumnOrSuperColumn cosc : key.getColumns()) {
                    matches.add(new Path(ByteBufferUtil
                            .string(cosc.column.value)));
                }
            }
    ...
    public Set<Path> listSubPaths(Path path) throws IOException {
        ...
        indexExpressions.add(new IndexExpression(sentCol, IndexOperator.EQ,
                sentinelValue));
        indexExpressions.add(new IndexExpression(parentPathCol,
                IndexOperator.EQ, ByteBufferUtil.bytes(startPath)));
    ...
    public BlockLocation[] getBlockLocation(List<Block> blocks, long start,
        ...
        try {
            List<List<String>> blockEndpoints =
                    ((Brisk.Iface) client).describe_keys(keySpace, blockKeys);
            ...
            for (int i = 0; i < blockEndpoints.size(); i++) {
                ...
                locations[i] =
                        new BlockLocation(null,
                                endpoints.toArray(new String[0]), offset,
                                b.length);
            }
